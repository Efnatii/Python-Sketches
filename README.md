...
Ключи для сторонних сервисов должны передаваться через переменные окружения. Для
удобства создайте файл `.env` (пример в `.env.example`) и загрузите его с
помощью `python-dotenv` или аналогичного инструмента.

**Никогда не храните реальные ключи в репозитории.**
